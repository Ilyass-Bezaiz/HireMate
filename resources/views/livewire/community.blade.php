  ...
  <div class="flex items-align justify-start gap-x-8 w-full mt-[32px]">
    ...
    <div
      ...
      <form wire:submit.prevent="" enctype="multipart/form-data">
        @csrf
        <div class="mb-6">
          <label for="categoryFilter" class="block text-[20px] font-medium text-gray-700 dark:text-gray-300 mb-2">Filter
            by category</label>
          @if($categories)
        ...
        <div class="mb-6">
          ...
          <select wire:model="selectedDate" id="dateFilter" name="dateFilter"
            class="mt-1 block w-[350px] py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
      ...
      <div class="w-full bg-white p-7 rounded-md border-[1px] border-[#d9d9d9]">
        <form enctype="multipart/form-data">
          ...
          <div class="mb-6 width-[800px]">
            ...
            <input wire:model="title" placeholder="Title" type="text" id="post-title" name="post-title"
              class="block w-full bg-white border border-gray-300 rounded-md py-2 px-3 text-base focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm sm:leading-5" />
            ...
            <select wire:model="selectedCategory" id="category" name="category"
              class="mt-4 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
              ...
              <textarea wire:model="description" placeholder="What's on your mind?" id="description" rows="3"
                class="block w-full mt-4 bg-white border border-gray-300 rounded-md py-2 px-3 text-base focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm sm:leading-5 overflow-y-hidden"></textarea>
              @error('description') <span class="error text-red-600 text-[12px]">{{ $message }}</span> @enderror
            </select>
          </div>
        </form>
        <button
          class="w-1/3 rounded-full py-2 bg-[#4DD783] text-[white] border border-[1px] border-[#4DD783] hover:bg-green-500"
      ...
      <div class="my-6 p-6 bg-white h-auto rounded-md border-[1px] border-[#d9d9d9]">
          ...
          <div class="flex items-center">
            <div class="flex items-center justify-center">
              <div class="img-picture w-[60px] h-[60px] rounded-full overflow-hidden">
                ...
                <img src="{{ Storage::url($post->user->profile_photo_path) }}" alt="Profile picture"
            ...
            <div class="ml-2 flex flex-col items-start">
              <span class="font-bold">{{$post->user->name}} - <span
                  class="font-light text-[14px] text-[#4DD783]">{{ $post->category->name }}</span></span>
              <span class="text-[12px] text-[#888]">{{ $post->created_at->diffForHumans() }}</span>
            </div>
          </div>
          <div x-data="{ open: false }" class="relative inline-block text-left">
            <button @click="open = !open"
              class="bg-gray-200 text-gray-700 font-semibold w-[35px] h-[35px] flex items-center justify-center rounded-full inline-flex items-center hover:bg-gray-300">
              <span>
            ...
            <div x-show="open" @click.away="open = false"
              class="origin-top-right absolute right-0 mt-2 w-[120px] rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
        ...
        <div class="mt-6 flex flex-row items-center">
          <button wire:click="toggleLikePost({{$post->id}})"
            ...
            <svg wire:loading.delay.remove xmlns="http://www.w3.org/2000/svg" width="26" height="24" viewBox="0 0 26 24"
              ...
              <div class="flex items-center">
                ...
                <div class="ml-2 flex flex-col items-start">
                  <span class="font-bold">{{$comment->user->name}} - <span
                      class="font-light text-[14px] text-[#4DD783]">Business Consultant</span></span>
