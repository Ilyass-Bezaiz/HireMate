                ...
                @foreach ($posts as $key => $post)
                    <li wire:click="showPopUpDetails({{ App\Models\JobOfferPost::where('id', $post->post_id)->get()[0]->id }})"
                        ...
                        <div class="text-gray-400 font-thing p-0 mt-0 flex justify-between items-center">
                            <p>{{ App\Livewire\HomePage\JobseekerOffers::getPostPublishDay(App\Models\JobOfferPost::where('id', $post->post_id)->get()[0]) }}
                            </p>
                        </div>
                        <div class="flex gap-3">
                            ...
                            <span wire:loading.class='scale-90 opacity-50 duration-200'
                                ...
                                wire:click='addFav({{ App\Models\JobOfferPost::where('id', $post->post_id)->get()[0]->id }})'
                                class="rounded-full p-2 cursor-pointer border-1px border-green-400 hover:bg-gray-700">
                                <img src="{{ $this->likedPost(App\Models\JobOfferPost::where('id', $post->post_id)->get()[0]->id) ? 'images/ic-full-heart.png' : 'images/ic-empty-heart.png' }} "
                ...
                @foreach ($posts as $key => $post)
                    <li wire:click="showPopUpDetails({{ App\Models\JobSeekerPost::where('id', $post->post_id)->get()[0]->id }})"
                        ...
                        <div class="text-gray-400 font-thing p-0 mt-1 flex items-end">
                            <p>{{ App\Livewire\HomePage\JobseekerOffers::getPostPublishDay(App\Models\JobSeekerPost::where('id', $post->post_id)->get()[0]) }}
                            </p>
                        </div>
                        <div class="flex gap-3">
                            ...
                            <span wire:loading.class='scale-90 opacity-50 duration-200'
                                ...
                                class="rounded-full p-2 cursor-pointer border-1px border-green-400 hover:bg-gray-700">
                                <img src="{{ $this->likedPost(App\Models\JobSeekerPost::where('id', $post->post_id)->get()[0]->id) ? 'images/ic-full-heart.png' : 'images/ic-empty-heart.png' }} "
                                    alt="ic-heart" width="30" height="30">
                            </span>
                        </div>
                    </li>
